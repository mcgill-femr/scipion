...
class XmippProtSplitVolumeHierarchical(ProtAnalysis3D):
    ...
    def convertInputStep(self, particlesId, volId):
        ...
        if self._useSeveralClasses():
            ...
            self.writeInfoField(self._getExtraPath(), "sampling",
                                xmipp.MDL_SAMPLINGRATE, newTs)
            self.writeInfoField(self._getExtraPath(), "size", xmipp.MDL_XSIZE,
                                newXdim)
    ...
    def classifyOneGroup(self, projNumber, projMdBlock, projRef,
        ...
        for classNo in range(1, Nclasses + 1):
            ...
            mdClasses.setValue(xmipp.MDL_CLASS_COUNT, localImagesMd.size(),
                               newClassId)
    ...
    def splitVolumeStep(self):
        ...
        sizeMd = md.getSize(self._getDirectionalClassesFn())
        matrixCoOc=np.zeros((sizeMd, sizeMd))
        ...
        for i in range(self.Nrec):
            ...
            args = "-i %s --oroot %s --Niter %d --sym %s " % \
                   (self._getDirectionalClassesFn(), fnRoot,
                    self.Niter.get(), self.symmetryGroup.get())
            if fnMask != "":
            ...
            listObjRef1=[]
            for id1 in outMd:
                refObj = outMd.getValue(xmipp.MDL_REF2, id1)
        ...
        for i in range(sizeMd):
            for j in range(sizeMd):
                if matrixCoOc[i][j]==0:
        ...
        for row in md.iterRows(origMd):
            refIdx = origMd.getValue(xmipp.MDL_REF2, row.getObjId())
            if(listLabels[refIdx-1]==0):
                row.addToMd(defMd1)
            else:
                row.addToMd(defMd2)

        defMd1.write(self._getExtraPath("split_def1.xmd"))
        defMd2.write(self._getExtraPath("split_def2.xmd"))

        args = "-i %s -o %s --max_resolution 0.25 --sym %s -v 0" % \
               (self._getExtraPath("split_def1.xmd"),
                self._getExtraPath("split_def1.vol"), self.symmetryGroup.get())
        self.runJob("xmipp_reconstruct_fourier", args, numberOfMpi=1)

        args = "-i %s -o %s --max_resolution 0.25 --sym %s -v 0" % \
               (self._getExtraPath("split_def2.xmd"),
                self._getExtraPath("split_def2.vol"), self.symmetryGroup.get())
        self.runJob("xmipp_reconstruct_fourier", args, numberOfMpi=1)
    ...
    def createOutputStep(self):
        ...
        if self.splitVolume and self.directionalClasses.get() > 1:
            ...
            for i in range(2):
                vol = Volume()
                vol.setLocation(1,
                                self._getExtraPath("split_def%d.vol" % (i + 1)))
    ...
    def _updateClass(self, item):
        ...
        reprojection.setLocation(
            xmippToLocation(classRow.getValue(xmipp.MDL_IMAGE1)))
