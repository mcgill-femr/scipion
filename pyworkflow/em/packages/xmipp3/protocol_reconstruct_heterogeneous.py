...
from pyworkflow.utils.path import cleanPath, cleanPattern, makePath, copyFile, \
    moveFile
...
class XmippProtReconstructHeterogeneous(ProtClassify3D):
    ...
    def convertInputStep(self, inputParticlesId):
        ...
        self.writeInfoField(fnDir, "sampling", xmipp.MDL_SAMPLINGRATE,
                            TsCurrent)
    ...
    def globalAssignment(self, iteration):
        ...
        self.writeInfoField(fnDirCurrent, "angleStep", xmipp.MDL_ANGLE_DIFF,
                            float(angleStep))
        ...
        for i in range(1, self.getNumberOfReconstructedVolumes() + 1):
            ...
            if not exists(fnLocalStk):
                ...
                if row.containsLabel(xmipp.MDL_CTF_MODEL) or row.containsLabel(
                    ...
                    numberGroups = mdInfo.getValue(xmipp.MDL_COUNT,
                                                   mdInfo.firstObject())
                    ctfPresent = True
                else:
                    numberGroups = 1
        ...
        for i in range(1, self.getNumberOfReconstructedVolumes() + 1):
            ...
            for half in range(1, 3):
                ...
                hasCTF = row.containsLabel(
                    xmipp.MDL_CTF_DEFOCUSU) or row.containsLabel(
                    xmipp.MDL_CTF_MODEL)
    ...
    def _updateParticle(self, particle, row):
        ...
        row.setValue(xmipp.MDL_SHIFT_X,
                     row.getValue(xmipp.MDL_SHIFT_X) * self.scaleFactor)
        row.setValue(xmipp.MDL_SHIFT_Y,
                     row.getValue(xmipp.MDL_SHIFT_Y) * self.scaleFactor)
