 ...
 ***************************************************************************/
#include "mpi_volume_variability.h"
...
/*  constructor ------------------------------------------------------- */
ProgMPIVolVariability::ProgMPIVolVariability(int argc, char *argv[])
{
    this->read(argc, argv);
}
 ...
 */
ProgMPIVolVariability::ProgMPIVolVariability(MpiNode * node)
{
    this->setNode(node);
}

/* Special way of reading to sync all nodes */
void ProgMPIVolVariability::read(int argc, char** argv)
{
    XmippMpiProgram::read(argc, argv);
    ProgVolVariability::read(argc, (const char **)argv);
}

/* Usage ------------------------------------------------------------------- */
void ProgMPIVolVariability::defineParams()
{
	ProgVolVariability::defineParams();
    ...
    addParamsLine("                                : possible value that may not be the best option");
}

/* Read parameters --------------------------------------------------------- */
void ProgMPIVolVariability::readParams()
{
	ProgVolVariability::readParams();
    mpi_job_size=getIntParam("--mpi_job_size");
}

/* Pre Run PreRun for all nodes but not for all works */
void ProgMPIVolVariability::preRun()
{
    if (nProcs < 2)
    ...
    {
        produceSideinfo(); //Aqui parece que todos los MPIs hacen trannsformaciones de Fourier. Esto colapsa con linea 100
        SF.firstObject();
    }
...
/* Run --------------------------------------------------------------------- */
void ProgMPIVolVariability::run()
{
    preRun();
    ...
    std::cout << " I am node : " << node->rank << std::endl;
    ...
    if( numberOfJobs < nProcs )
    {
        ...
        std::cout << " node->rank : " << node->rank  << " active? " <<node->active << std::endl;
    ...
    for (int i=0;i<(int)nProcs;i++) //algunos prcos estan inactivos
        ranks[i]=i+1;
...
#undef DEBUG
     ...
     #undef DEBUG
                ...
                if (status.MPI_TAG == TAG_COLLECT_FOR_FSC) //MIELDA FSC
                {
...
int  ProgMPIVolVariability::sendDataInChunks( double * pointer, int dest, int totalSize, int buffSize, MPI_Comm comm )
{
